...
Опційний масив, що розміщує на полі перешкоди на фіксованих клітинках:

```json
"obstacles": [
  { "type": "ice",   "cells": [[2,2],[3,3]] },
  { "type": "box",   "cells": [[1,5],[4,1]] },
  { "type": "burnt", "cells": [[5,5]] }
]
```

- **ice (лід)** — напівпрозоре покриття **поверх звичайного тайла** (кадр 16); тайл заморожений: не бере участі в матчах, свапах і гравітації. Знімається одним сусіднім матчем.
