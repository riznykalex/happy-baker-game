...
Останнє оновлення: інструменти 3+ (спрощено), усі написи та значки ×3, торт перезапускає таймер.
...
| `BOARD_W/H` | **896** (7×128) |
| `GAME_W` | 960 |
| `GAME_H` | ~1936 (BOARD_OFFSET_Y 700 + поле 896 + хелпери 340) |
| HUD | білий блок зверху 700px, іконки лічильників **144px** (сітка 2×2), шрифти 66–108px |
| Helpers | кола r=120, зображення **216px**, % 48px |
...
├────────────────────────────────────────┤
│  🦉 %    🦊 %    🐉 %           Log   │
│  (кола r=120, зображення 216px)        │
└────────────────────────────────────────┘
```

HUD — білий блок зверху (висота 700px), лічильники інгредієнтів оновлюються з матчів FLOUR/MILK/BUTTER/ягід. Квоти беруться з `levels.json → ingredient_goals` (за замовчуванням: flour 10, milk 3, butter 5, berries 5). Лічичник, що досягнув квоти, зеленіє з `✓`.
