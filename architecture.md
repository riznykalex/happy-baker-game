...
Останнє оновлення: торт перезапускає таймер, інструменти по рівнях, стартові оверлеї (знайомство + завдання), інгредієнти→торт з анімацією, більший мобільний HUD.
...
| `BOARD_W/H` | **896** (7×128) |
| `GAME_W` | 960 |
| `GAME_H` | ~1222 (BOARD_OFFSET_Y 176 + поле 896 + хелпери 150) |
| HUD | білий блок зверху 168px, іконки лічильників 48px, шрифти 22–36px |
| Helpers | кола r=44, зображення 72px, % знизу |
...
│          ⏱ 01:24  (28px)              │
│  🌾 10/10✓ 🥛 3/3✓ 🧈 5/5  🍓 5/5   │
│   (іконки 48px, ✓ при досягненні квоти)│
├────────────────────────────────────────┤
│          поле 896×896                  │
│      (рамка, без bg.jpg)               │
├────────────────────────────────────────┤
│  🦉 %   🦊 %   🐉 %            Log    │
│  (кола r=44, зображення 72px)          │
└────────────────────────────────────────┘
```

HUD — білий блок зверху (висота 168px), лічильники інгредієнтів оновлюються з матчів FLOUR/MILK/BUTTER/ягід. Квоти беруться з `levels.json → ingredient_goals` (за замовчуванням: flour 10, milk 3, butter 5, berries 5). Лічичник, що досягнув квоти, зеленіє з `✓`.
