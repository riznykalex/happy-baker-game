...
Останнє оновлення: інструменти 3+, написи та іконки ×2.25, лічильники інгредієнтів в один рядок, торт перезапускає таймер.
...
| `BOARD_W/H` | **896** (7×128) |
| `GAME_W` | 960 |
| `GAME_H` | ~1551 (BOARD_OFFSET_Y 400 + поле 896 + хелпери 255) |
| HUD | білий блок зверху 400px, лічильники інгредієнтів **в один рядок 1×4** (іконки 96px), шрифти 48–80px |
| Helpers | кола r=90, зображення **162px**, % 36px |
...
> **Таймер**: кожен спечений торт (будь-яким шляхом) повністю перезапускає таймер на `timer_seconds` — arcade-режим «встигни спекти торт за час». Логування: `timer_reset`.
...
На старті рівня послідовно показуються оверлеї: спершу знайомство з новими інструментами (зображення + ефект), потім завдання (торт + необхідна кількість). Поки оверлеї видно — таймер не запущено, поле заблоковане.
...
**Підказка (hint)**: якщо гравець не ходить > 5 сек — `checkIdleHint` знаходить валідний своп через `findHintMove()` і підсвічує жовтим пару тайлів + toast. Ховається при будь-якому тапі (`hideHint`).
...
├────────────────────────────────────────┤
│  🦉 %    🦊 %    🐉 %           Log   │
│  (кола r=90, зображення 162px)        │
└────────────────────────────────────────┘
```

HUD — білий блок зверху (висота 400px), лічильники інгредієнтів оновлюються з матчів FLOUR/MILK/BUTTER/ягід. Квоти беруться з `levels.json → ingredient_goals` (за замовчуванням: flour 10, milk 3, butter 5, berries 5). Лічичник, що досягнув квоти: іконка сіріє (tint), поруч з'являється зелена `✓`.
...
`ingredient_goals` та `tools` опціональні: без `ingredient_goals` діють дефолти (flour 10, milk 3, butter 5, berries 5), без `tools` інструменти не спавняться.
